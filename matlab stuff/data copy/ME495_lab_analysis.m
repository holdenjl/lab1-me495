%% ME495: measured data, physical model, sensitivity and four design studies
...
B_tr_selected = 0.001620;       % N*m*s/rad, Bs + Bb
B_tl_selected = 0.006804;       % N*m*s/rad, Bs + Bm
...
outFolder = fullfile(dataFolder,'ME495_results');
...
for k=1:numel(listing)
    if listing(k).isdir || contains(listing(k).folder,outFolder), continue; end
...
p=struct('Km',0.097,'Rm',1.6,'Lm',0.002,'Jm',4.23e-5, ...
    ...
    'Gs',7.31e10,'Bs',NaN,'Bb',NaN,'Bm',NaN,'scaleK',1);
...
fprintf('SELECTED sums used below: Btr=%.8g, Btl=%.8g N*m*s/rad\n',B_tr_selected,B_tl_selected);
...
for k=1:numel(runs)
    ...
    A=runs(k).A;t=A(:,1)-A(1,1);y=A(:,4);
    direction=sign(median(y));if direction==0,continue;end;y=direction*y;
    keep=y>0; t=t(keep);y=y(keep);
    ...
    coastRows(end+1,:)={runs(k).name,alpha,B,Tc,rmse(y,ex),rmse(y,linear),lin(1),numel(y)}; %#ok<SAGROW>
    fig=figure('Color','w');plot(t,y,'Color',[.65 .65 .65]);hold on;plot(t,ex,'b-',t,linear,'r--','LineWidth',1.3);
    grid on;xlabel('Time (s)');ylabel('Distal speed (rad/s)');legend('Measured','Viscous exponential','Coulomb linear');
    title(runs(k).name,'Interpreter','none');saveas(fig,fullfile(figFolder,sprintf('coast_%02d.png',k)));
end
writetable(cell2table(coastRows,'VariableNames',{'File','Alpha_per_s','EffectiveBb_Nm_s_rad','CoulombTorque_Nm','ViscousRMSE_rad_s','CoulombRMSE_rad_s','LinearSlope_rad_s2','Samples'}),fullfile(outFolder,'coastdown.csv'));
